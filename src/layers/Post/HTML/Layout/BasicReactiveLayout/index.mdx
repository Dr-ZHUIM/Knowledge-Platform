...
根据 <Link target="_blank" to="https://caniuse.com/css-grid">caniuse</Link> 网站的统计，CSS Grid 能够支持 97.8% 的用户，Flex 布局仅仅比这个比重高了 0.5%
...
一个 `display` 属性为 `grid` 的容器，默认只有一列，每次添加元素都会占据新的一行。

<GridDefaultDemo />

可以使用 `grid-template-columns` 与 `grid-template-rows` 来规定列数与行数，其中被分划的格子就是子元素所处的位置
