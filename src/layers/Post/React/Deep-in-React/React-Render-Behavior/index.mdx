...
| 渲染输出 / React.Element / Fiber Node | React 使用 React.createElement 生成的描述 DOM 结构的对象，是 Fiber 结构的节点 |
...
默认情况下，每进行一次 `setState` 都会导致 React 开始一次新的渲染队列，同步的执行渲染行为，最终返回渲染结果。

然而，React 在内部也应用了一渲染队列优化手段 —— 批处理。
