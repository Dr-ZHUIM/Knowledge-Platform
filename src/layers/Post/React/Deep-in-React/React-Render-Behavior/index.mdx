...
应当注意的是 `key` **不是一个真实的属性**，对 React 来说，`key` 只是一个提醒用的属性，会将其消除，因此**用户无法获取它**。
...
也就是说，**key 的修改会导致组件重新渲染**，用户可以通过为组件绑定一个 key，然后通过改变这个 key 而重新渲染这个组件。
...
接下来本文将从三个层面来解释这个问题，理念层 —— State Snapshot 以及 原理层 -- React 渲染是同步还是异步的？ —— 实现层 闭包函数 Event Handler

#### State 快照

> State 变量不像你在 JS 中可以读写的变量，它表现地更像一个快照。setState 不会修改用户已经维护好的 state，而是会触发一次重渲染 —— React 官方

用户调用的 state 实际上并没有维护在组件中，state 维护在 React 内部， 当 React 调用了用户的渲染函数, 渲染函数返回的 JSX 就像 UI 当前所有 state 的快照。它的 props，Event Handler 以及 内部变量都会使用这个快照里的 state 进行运算。
...
也就是说，用户调用的 state 的值就是上次渲染中 state 的渲染快照，**state 只会在重渲染的时候被更新**
...
本节讨论的异步并不是程序意义上的异步，而是指 **setState 后是否立刻进行渲染行为**
